...
zfs_fiemap_visit_indirect(spa_t *spa, const dnode_phys_t *dnp,
    blkptr_t *bp, const zbookmark_phys_t *zb, blkptr_cb_t func, void *arg)
{
	int error = 0;
...
zfs_fiemap_fill_next_extent(zfs_fiemap_t *fm, struct fiemap_extent_info *fei,
    uint64_t logical_start, uint64_t physical_start, uint64_t logical_len,
    uint64_t physical_len, uint32_t device, uint32_t flags)
{
...
zfs_fiemap_create(uint64_t start, uint64_t len, uint64_t flags, uint64_t max)
{
	...
	fm->fm_dirty_tree = range_tree_create(NULL, RANGE_SEG64, NULL, 0, 0);
	fm->fm_free_tree = range_tree_create(NULL, RANGE_SEG64, NULL, 0, 0);
